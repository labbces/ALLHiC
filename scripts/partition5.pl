...
if ((!defined $opt_g)|| (!defined $opt_r)) {
    ...
    Usage: perl $0 -g Allele.gene.table -r draft.asm.fasta
      ...
      -t : optional, number of cores/CPUs, default 1
************************************************************************\n";
}
...
#		print "$chrn @ctgdb\n";
	...
	system("samtools view --regions-file $wrkd/$chrn/seq.fasta.bed $bam| awk 'FNR==NR { a[\$NF]; next } \$7 in a ||  \$7 == \"=\"' $wrkd/$chrn/ctg.list /dev/stdin > $wrkd/$chrn/sample.clean.sam");
